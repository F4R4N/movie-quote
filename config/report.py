...
while True:
    ...
    if datetime.datetime.now().strftime("%Y%m%d%H%M%S") == datetime.datetime(datetime.datetime.now().year, datetime.datetime.now().month, 8, 22, 53, 59).strftime("%Y%m%d%H%M%S"):
        monthly_report()
        time.sleep(1)
    else:
        time.sleep(1)
